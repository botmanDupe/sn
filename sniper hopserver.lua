...
local function jumpToServer()
    ...
    while true do
        ...
        for _, server in ipairs(serverList.data) do
            if server.playing and tonumber(server.playing) >= MINIMUM_PLAYERS and tonumber(server.playing) < Players.MaxPlayers and not table.find(ServerHopData.CheckedServers, tostring(server.id)) and server.id ~= game.JobId and server.playing >= 1 then
                ...
                -- Check if trade server is full
                if server.playing >= Players.MaxPlayers then
                    -- Redirect back to the original server
                    TeleportService:TeleportToPlaceInstance(PlaceId, game.JobId, game.Players.LocalPlayer)
                    return
                end
